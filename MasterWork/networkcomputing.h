...
int nInput = 786; //number of input neurons
int nHidden = 30;//number of hidden layer neurons
...
MatrixXd wInputHidden(nInput, nHidden);
MatrixXd wHiddenOutput(nHidden, nOutput); //matrices of weights between different layers

MatrixXd deltaHiddenOutput = MatrixXd::Zero(nHidden, nOutput);
MatrixXd deltaInputHidden = MatrixXd::Zero(nInput, nHidden);
...
MatrixXd sigmoid_matirx(MatrixXd f)
{
	...
	f.unaryExpr(sigmoid_matrix_ptr);
	return f;
}
